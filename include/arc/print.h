...
#define error(str, ...) { u32 size = 13 + calcf(str, ##__VA_ARGS__); s8 newStr[size]; strcpy(newStr, "[Arc] ERROR: "); strcat(newStr, str); s8 buf[size]; sprintf(buf, newStr, ##__VA_ARGS__); _printk(buf); }
